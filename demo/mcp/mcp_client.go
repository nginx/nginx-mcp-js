...
func main() {
	...
	toolDist := NewToolDistribution(toolNames)
	...
	for i := 0; i < config.Workers; i++ {
		...
		go func(workerID int, profile ClientProfile) {
			defer wg.Done()
			runWorker(ctx, config, toolDist,
				clientStats[profile.Name], workerID, profile)
		}(i, assignments[i])
	}
...
func runWorker(ctx context.Context, cfg *Config, tools *ToolDistribution, stats *Stats, workerID int, profile ClientProfile) {
	...
	if err != nil {
		log.Printf("Worker %d [%s]: failed to connect: %v", workerID, profile.Name, err)
		return
	}
	defer session.Close()
	...
	for {
		...
		// Estimate received bytes from content
